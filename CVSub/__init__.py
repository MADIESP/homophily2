...
def creating_vec_matching(player):
    vec_matching = []
    vec_matching.extend((player.id_in_group, player.rank1st, player.rank2nd, player.rank3rd, player.rank4th))
...
def creating_group_with_choices_round_1(group):
    ...
    for player in group.get_players():
        if player.InGroupA==True:
            Group_A_ch.append((player.id_in_group, (groupB[player.rank1st - 1],
                                                    groupB[player.rank2nd - 1],
                                                    groupB[player.rank3rd - 1],
                                                    groupB[player.rank4th - 1])))
        elif player.InGroupB==True:
            Group_B_ch.append((player.id_in_group, (groupA[player.rank1st - 1],
