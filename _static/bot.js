	...
	if(name!="-hidden-noname-") { //name!="_"
		if(form_map[name].length>1) {
			if(form_map[name][0].type == "checkbox") {
				...
				/*if(name_inputs[choosen].type == "text") {
							var min=0, max=100;
							if(name_inputs[choosen].hasAttribute("min")) min=form_map[name][i].min;
							if(name_inputs[choosen].hasAttribute("max")) max=form_map[name][i].max;
							var choosen=parseFloat(min)+Math.floor(Math.random()*(parseFloat(max)+1-parseFloat(min)));
							$(name_inputs[choosen]).val(choosen); console.log("value set",name);
							console.log(name_inputs[choosen], choosen, min, max);
				}*/
...
$(document).ready(function() {if(_bot_input_forms_passed==0) {
