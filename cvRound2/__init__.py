...
class C(BaseConstants):
    NAME_IN_URL = 'cvRound2'
    PLAYERS_PER_GROUP = None
    NUM_ROUNDS = 1
    Fnames = ["Charlotte", "Olivia", "Sophia", "Amelia"]
    Mnames = ["Liam", "Noah", "William", "Henry"]
...
class Player(BasePlayer):
    ...
    chosen_nameF = models.StringField(
        choices=C.Fnames,
        label="Please select a name that will be assigned to you for Part 4 of the experiment:",
        initial="",
    )
...
def Fnames(player):
    if player.chosen_nameF == "Charlotte":
        FemaleNames = 1
    elif player.chosen_nameF == "Olivia":
        FemaleNames = 2
    elif player.chosen_nameF == "Sophia":
        FemaleNames = 3
...
def Femalename(player):
    player.participant.FemaleNames_round2 = player.FemaleNames
    player.participant.name_round2 = player.participant.FemaleNames_round2


def Malename(player):
    player.participant.MaleNames_round2 = player.MaleNames
    player.participant.name_round2 = player.participant.MaleNames_round2
...
def creating_CV_ind(player):
    ...
    if player.participant.gender == 0:
        vec_cv.extend(
            (player.id_in_group, player.FemaleNames, player.participant.College, player.participant.BornPA, player.participant.Job, player.participant.gender))
    elif player.participant.gender == 1:
        vec_cv.extend((player.id_in_group, player.MaleNames, player.participant.College, player.participant.BornPA, player.participant.Job, player.participant.gender))
    player.vec_cv = str(vec_cv)
    player.participant.vec_cv_round2 = player.vec_cv
...
def creatingGroups(group):
    ...
    for player in group.get_players():
        if player.participant.gender == 1:
            if sum_hA < 2:
                ...
                player.participant.InGroupA_round2 = True
                sum_hA += 1
            else:
                player.InGroupA = False
                player.participant.InGroupA_round2 = False
        else:
            if sum_fA < 2:
                Group_A.append(player.id_in_group)
                cvA.append(player.vec_cv)
    ...
    group.session.Group_A_round2 = str(Group_A)
    group.cvA = str(cv_A)
    group.session.cvA_round2 = str(cv_A)
        ...
        if player.InGroupA == False:
            if player.participant.gender == 1:
                if sum_hB < 2:
                    ...
                    player.participant.InGroupB_round2 = True
                    sum_hB += 1
                else:
                    player.InGroupB = False
                    player.participant.InGroupB_round2 = False
            else:
                if sum_fB < 2:
    ...
    group.session.Group_B_round2 = str(Group_B)
    group.cvB = str(cv_B)
    group.session.cvB_round2 = str(cv_B)
    group.cv_A_1 = str(cvA[0])
    group.session.cv_A_1_round2 = str(cv_A[0])
    group.cv_A_2 = str(cvA[1])
    group.session.cv_A_2_round2 = str(cv_A[1])
    group.cv_A_3 = str(cvA[2])
    group.session.cv_A_3_round2 = str(cv_A[2])
    group.cv_A_4 = str(cvA[3])
    group.session.cv_A_4_round2 = str(cv_A[3])
    group.cv_B_1 = str(cvB[0])
    group.session.cv_B_1_round2 = str(cv_B[0])
    group.cv_B_2 = str(cvB[1])
    group.session.cv_B_2_round2 = str(cv_B[1])
    group.cv_B_3 = str(cvB[2])
    group.session.cv_B_3_round2 = str(cv_B[2])
    group.cv_B_4 = str(cvB[3])
    group.session.cv_B_4_round2 = str(cv_B[3])
...
def create_image_cv(cv_1, cv_2, cv_3, cv_4):
    ...
    answers = [
        [["Charlotte", "Liam"], ["Olivia", "Noah"], ["Sophia", "William"], ["Amelia", "Henry"]],
        ["Licence", "Master"],
        ["No", "Yes"],
        ["No", "Yes"]
...
def image_cv(group):
    cvA1 = eval(group.session.cv_A_1_round2)
    cvA2 = eval(group.session.cv_A_2_round2)
    cvA3 = eval(group.session.cv_A_3_round2)
    cvA4 = eval(group.session.cv_A_4_round2)
    cvB1 = eval(group.session.cv_B_1_round2)
    cvB2 = eval(group.session.cv_B_2_round2)
    cvB3 = eval(group.session.cv_B_3_round2)
    cvB4 = eval(group.session.cv_B_4_round2)
    ...
    static_path = otree.settings.STATIC_ROOT
    image_A_path = os.path.join(static_path, 'ImageA_round2.png')
    image_B_path = os.path.join(static_path, 'ImageB_round2.png')
...
class NameSelectionF(Page):
    ...
    def is_displayed(player):
         return player.chosen_nameF == "" and player.participant.gender==0
...
class NameSelectionM(Page):
    ...
    def is_displayed(player):
         return player.chosen_nameM == "" and player.participant.gender==1
