...
class C(BaseConstants):
    NAME_IN_URL = 'cvRound3'
    PLAYERS_PER_GROUP = None
    NUM_ROUNDS = 1
    Fnames = ["Jessica", "Sarah", "Karen", "Lisa"]
    Mnames = ["Thomas", "Christopher", "Charles", "Daniel"]
...
class Player(BasePlayer):
    ...
    chosen_nameF = models.StringField(
        choices=C.Fnames,
        label="Please select a name that will be assigned to you for part 5 of the experiment:",
        initial="",
    )
...
def Fnames(player):
    if player.chosen_nameF == "Jessica":
        FemaleNames = 1
...
def Mnames(player):
    if player.chosen_nameM == "Thomas":
        MaleNames = 1
    elif player.chosen_nameM == "Christopher":
        MaleNames = 2
    elif player.chosen_nameM == "Charles":
        MaleNames = 3
    elif player.chosen_nameM == "Daniel":
        MaleNames = 4
...
def Femalename(player):
    player.participant.FemaleNames_round3 = player.FemaleNames
    player.participant.name_round3 = player.participant.FemaleNames_round3


def Malename(player):
    player.participant.MaleNames_round3 = player.MaleNames
    player.participant.name_round3 = player.participant.MaleNames_round3
...
def creating_CV_ind(player):
    ...
    player.participant.vec_cv_round3 = player.vec_cv
...
def creatingGroups(group):
    ...
    for player in group.get_players():
        if player.participant.gender == 1:
            if sum_hA < 2:
                ...
                player.participant.InGroupA_round3 = True
                sum_hA += 1
            else:
                player.InGroupA = False
                player.participant.InGroupA_round3 = False
        else:
            if sum_fA < 2:
                Group_A.append(player.id_in_group)
                cvA.append(player.vec_cv)
    ...
    group.session.Group_A_round3 = str(Group_A)
    group.cvA = str(cv_A)
    group.session.cvA_round3 = str(cv_A)
        ...
        if player.InGroupA == False:
            if player.participant.gender == 1:
                if sum_hB < 2:
                    ...
                    player.participant.InGroupB_round3 = True
                    sum_hB += 1
                else:
                    player.InGroupB = False
                    player.participant.InGroupB_round3 = False
            else:
                if sum_fB < 2:
    ...
    group.session.Group_B_round3 = str(Group_B)
    group.cvB = str(cv_B)
    group.session.cvB_round3 = str(cv_B)
    group.cv_A_1 = str(cvA[0])
    group.session.cv_A_1_round3 = str(cv_A[0])
    group.cv_A_2 = str(cvA[1])
    group.session.cv_A_2_round3 = str(cv_A[1])
    group.cv_A_3 = str(cvA[2])
    group.session.cv_A_3_round3 = str(cv_A[2])
    group.cv_A_4 = str(cvA[3])
    group.session.cv_A_4_round3 = str(cv_A[3])
    group.cv_B_1 = str(cvB[0])
    group.session.cv_B_1_round3 = str(cv_B[0])
    group.cv_B_2 = str(cvB[1])
    group.session.cv_B_2_round3 = str(cv_B[1])
    group.cv_B_3 = str(cvB[2])
    group.session.cv_B_3_round3 = str(cv_B[2])
    group.cv_B_4 = str(cvB[3])
    group.session.cv_B_4_round3 = str(cv_B[3])
...
def image_cv(group):
    cvA1 = eval(group.session.cv_A_1_round3)
    cvA2 = eval(group.session.cv_A_2_round3)
    cvA3 = eval(group.session.cv_A_3_round3)
    cvA4 = eval(group.session.cv_A_4_round3)
    cvB1 = eval(group.session.cv_B_1_round3)
    cvB2 = eval(group.session.cv_B_2_round3)
    cvB3 = eval(group.session.cv_B_3_round3)
    cvB4 = eval(group.session.cv_B_4_round3)
    ...
    static_path = otree.settings.STATIC_ROOT
    image_A_path = os.path.join(static_path, 'ImageA_round3.png')
    image_B_path = os.path.join(static_path, 'ImageB_round3.png')
